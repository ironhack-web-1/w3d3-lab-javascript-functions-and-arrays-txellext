...
function sumNumbers(numbersArray) {
  ...
  for (i = 0; i < numbersArray.length; i++) {
    sumNumbersArray += numbersArray[i];
  }
...
function averageWordLength(arrayOfWords) {
  ...
  for (i = 0; i < arrayOfWords.length; i++) {
    if (arrayOfWords[i] !== 0);
    sumWordLength += arrayOfWords[i].length;
  }
  return sumWordLength / arrayOfWords.length;
}
...
function uniquifyArray(wordsArray) {
  ...
  const filteredArray = wordsArray.filter(
    (item, position) => wordsArray.indexOf(item) === position
  );
...
//console.log(doesWordExist(wordsFind, 'hola'));
...
function howManyTimes(wordsArray, word) {
  return wordsArray.length === 0
    ? null
    : wordsArray.filter((item) => item === word).length;
}
